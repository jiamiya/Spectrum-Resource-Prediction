...
class Freqpred:
    ...
    def test(self):
        ...
        print(">> Acc:", acc)
        print(">> Conflict_rate:", conflict_rate1, conflict_rate2)
    ...
    def compute_conflict(self, pred, gt): # # (time, bin_size) pred (117, 128) gt
        ...
        gt_thre = 0.2
        for i in range(time):
            s = np.exp(p[i]) / np.sum(np.exp(p[i]))
            # 用不占用率来选取bin
            ...
            for j in range(bin_size):
                ...
                conflict1 += p_gt * 0.5 * (gt[i][j] > gt_thre)
                # conflict2 += ((1-pred[i][j])/total) * (1-pred[i][j])*(gt[i][j] > gt_thre)
                conflict2 += s[j] * 0.5 * (gt[i][j] > gt_thre)

                # 不考虑这个bin里发不发
                conflict3 += p_gt * (gt[i][j] > gt_thre)
                conflict4 += s[j] * (gt[i][j] > gt_thre)
            """
            total = bin_size-np.sum(pred[i])
            conflict1, conflict2, conflict3, conflict4= 0,0,0,0
            for j in range(bin_size):
                # 选哪个bin * 这个bin里面发不发* 这个bin里发送冲不冲突
                conflict1 += p_gt*0.5*(gt[i][j] > gt_thre)
                # conflict2 += ((1-pred[i][j])/total) * (1-pred[i][j])*(gt[i][j] > gt_thre)
                conflict2 += ((1 - pred[i][j]) / total) * 0.5 * (gt[i][j] > gt_thre)
                conflict3 += p_gt * (gt[i][j] > gt_thre)
                conflict4 += ((1-pred[i][j])/total) * (gt[i][j] > gt_thre)
            """
        ...
        print('3,4', ave_conflict3/time, ave_conflict4/time)
        return ave_conflict1/time, ave_conflict2/time
